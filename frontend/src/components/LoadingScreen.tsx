...
export default function LoadingScreen({ onComplete }: { onComplete: () => void }) {
  ...
  // Scroll terminal to bottom on change
  useEffect(() => {
  ...
  useEffect(() => {
    if (currentIndex < BOOT_LOGS.length) {
      ...
      // Calculate delay based on log type to look realistic
      let delay = 150 + Math.random() * 200
      if (log.type === 'process') delay = 400 // processes take longer
      if (log.type === 'divider') delay = 50

      const timer = setTimeout(() => {
        setLogs(prev => [...prev, log.text])
        soundService.playHover() // play tiny tick for terminal line
        setCurrentIndex(prev => prev + 1)
      }, delay)

      return () => clearTimeout(timer)
    } else {
      // Finished printing boot logs. Pause before showing the prompt.
      const timer = setTimeout(() => {
  ...
  // Typwriter prompt input simulation
  useEffect(() => {
    ...
    const typingInterval = setInterval(() => {
      if (charIndex < fullCommand.length) {
        setTypedInput(prev => prev + fullCommand[charIndex])
        ...
        setTimeout(() => {
          ...
          setIsDone(true)
          setTimeout(onComplete, 800) // allow completion screen exit transition
        }, 600)
      }
    }, 250)
    ...
    <AnimatePresence>
      {!isDone && (
        ...
        >
          ...
          {/* Cinematic background blob glow */}
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] rounded-full opacity-10 blur-[150px] pointer-events-none"
               style={{ background: 'radial-gradient(circle, #8b5cf6 0%, #06b6d4 100%)' }} />
